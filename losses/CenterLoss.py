...
# I think this is exactly the KL divergence loss
class CenterLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        # get the all unique labels
        targets_ = list(set(targets.data.item()))
        num_class = len(targets_)

        targets_ = Variable(torch.LongTensor(targets_))
        # repeat num_class times in dimension 1 (add more rows)
