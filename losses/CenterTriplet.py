...
class CenterTripletLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        mask_ = mask_.type(torch.LongTensor)

        
        # print(mask_.size())
        _mask = Variable(torch.LongTensor(num_class, n).fill_(1)).cuda() - mask_
        centers = []
        inputs_list = []
        ...
        dist_an = torch.mean(neg_dist).data.item()
        dist_ap = torch.mean(pos_dist).data.item()
