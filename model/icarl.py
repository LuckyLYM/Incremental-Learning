...
class Net(torch.nn.Module):
    ...
    def observe(self, x, t, y):
        ...
        for iter in range(self.n_iter):
            ...
            # get loss on new data
            self.net.zero_grad()
            loss = self.bce((self.net(x)),y)

            # get soft loss on old data
            if self.num_exemplars > 0:
                ...
                #-----when they update mem_class_y-----#

                # strickly speaking, we are not allowed to store the extra soft targtes, 
                # since we have limited buffer size
                # sample soft target from previous data
                for cc in self.mem_class_x.keys():
                ...
                loss += self.reg * self.kl(self.lsm(self.net(inp_dist)),self.sm(target_dist))
        ...
        if self.examples_seen == self.samples_per_task:
            ...
            for ll in range(num_classes):
                ...
                cdata = self.memx.index_select(0, indxs) # dimension,index
                ...
                for ee in range(self.num_exemplars):
                    ...
                    cost = (mean_feature.expand(ntr, nd) - (model_output + prev.expand(ntr, nd)) / (ee + 1)).norm(2, 1).squeeze()
                    _, indx = cost.sort(0)
                    winner = 0
