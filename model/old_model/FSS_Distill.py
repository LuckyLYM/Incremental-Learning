...
'''
    stream clustering equipped with distillation techniques.
    the code is based on FSS_Multi
'''
...
class Net(nn.Module):
    ...
    def cross_distillation_loss(self, outputs, labels, teacher_outputs):
    ...
    def observe(self, x, t, y, pretrained=None):
        ...
        if t!=self.old_task:
            ...
            if self.n_task>1:
                ...
                # check distillation and frozen the model
                if self.distill:
                    self.update_frozen_model()
                    print('use model distillation')

            # intialize the cluster setting
            self.sampled_memory_data=None
            self.sampled_memory_labs=None
            self.cluster_distance=0
        ...
        for iter_i in range(self.n_iter):
            ...
            self.zero_grad()
            output_batch=self.forward(x)

            if not self.distill or self.n_task==1:
                loss = self.ce(output_batch, y)
                loss.backward()
                self.opt.step()
            ...
            if self.n_task>1:
                ...
                #----improvement needed
                # if we store the teacher outputs, the code may be faster

                else:
                    output_teacher_batch=self.teacher.forward(batch_x)
                    loss=self.cross_distillation_loss(output_batch,y,output_teacher_batch)
