...
'''
    new feature: modify the k-center algorithm to make sure that we 
    always have exact k samples in the buffer
'''
...
class Net(nn.Module):
    ...
    # modify the stream k-center algorithm
    def select_k_centers(self,pretrained,beta=2,alpha=1):
        ...
        # add points that are beyond current cluster distance
        added_indes=[]
        ...
        if (len(added_indes)+self.sampled_memory_data.size(0))>self.task_buffer_size:
            ...
            # get the distance matrix of all data
            ...
            while est_mem_size>self.task_buffer_size:
                # double the cluster distance
                self.cluster_distance=self.cluster_distance*beta

                # random sample an initial point
                first_ind=torch.randint(0,init_points_feat.size(0),(1,))[0]
                cent_inds=[first_ind.item()]  # .item() returns a specific value
            ...
            #-----new componet here-----#
            total_size=init_points_feat.size(0)
            new_size=len(cent_inds)
            if new_size<self.task_buffer_size:
                residual=self.task_buffer_size-new_size
                diff=list(set(range(total_size))-set(cent_inds))
                random_inds=random.sample(range(0,len(diff)),residual)
                residual_inds=[diff[i] for i in random_inds]
                cent_inds.extend(residual_inds)
        ...
        if t!=self.old_task:
            ...
            if self.n_task>1:
                ...
                for index in range(self.n_old_task):
                    ...
                    # use new shirk size strategy, hope this helps
                    self.sampled_task_data[task_id]=self.sampled_task_data[task_id][-self.task_buffer_size:]
                    self.sampled_task_labs[task_id]=self.sampled_task_labs[task_id][-self.task_buffer_size:]
