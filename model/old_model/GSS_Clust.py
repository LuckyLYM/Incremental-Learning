...
def add_memory_grad(pp, mem_grads, grad_dims):
    ...
    """
    # gather the gradient of the new memory in one vector
...
def add_group_memory_grad(pp, mem_grads, grad_dims, size):
    """
        This stores the gradient of a new memory and compute the dot product with the previously stored memories.
        pp: parameters
        mem_grads: gradients of previous memories
    ...
    """
    # gather the gradient of the new memory in one vector
    grads = get_group_grad_vector(pp, grad_dims, size)
    if mem_grads is None:
        mem_grads = grads
    else:
        grads = grads
...
def get_group_grad_vector(pp, grad_dims, size):
    """
     gather the gradients in one vector
    """
    grads = torch.Tensor((size,sum(grad_dims)))
    grads.fill_(0.0)
    cnt = 0
    for param in pp():
        if param.grad is not None:
            beg = 0 if cnt == 0 else sum(grad_dims[:cnt])
            en = sum(grad_dims[:cnt + 1])
            grads[:,beg: en].copy_(param.grad1.data.view(size,-1))
        cnt += 1
    return grads
...
class Net(nn.Module):
    ...
    def get_samples_gradients(self,mem_data,mem_labs):
        mem_grads=None
        #-------can we use the batch mode here?-------#
        #-------seems not-------#
    ...
    def get_group_samples_gradients(self,mem_data,mem_labs):
        mem_grads=None

        size=mem_data.size(0)
        self.zero_grad()
        ptloss = self.ce(self.forward(mem_data),mem_labs)
        ptloss.backward()
        autograd_hacks.compute_grad1(self.net)
        mem_grads = add_group_memory_grad(self.parameters, mem_grads, self.grad_dims, size)
        self.zero_grad()
        return mem_grads
    ...
    def select_k_centers(self,beta=0.1,alpha=1):
        ...
        new_mem_features = self.get_samples_gradients(new_memories_data,new_memories_labs)
        samples_mem_features = self.get_samples_gradients(self.sampled_memory_data,self.sampled_memory_labs)

        '''
        # enable hook
        autograd_hacks.add_hooks(self.net)

        new_mem_features = self.get_group_samples_gradients(new_memories_data,new_memories_labs)
        samples_mem_features = self.get_group_samples_gradients(self.sampled_memory_data,self.sampled_memory_labs)

        # disable hook
        autograd_hacks.remove_hooks(self.net)
        '''
