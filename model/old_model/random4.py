...
class Net(nn.Module):
    def __init__(self,
        ...
        self.sample_size_list_without_current_class=[]
    ...
    def observe(self, x, t, y, pretrained=None):
        ...
        if t!=self.old_task:
            ...
            remainder=self.n_constraints%self.n_task
            quotient=int(self.n_constraints/self.n_task)
            self.sample_size_list=[quotient for _ in range(self.n_task)]
            ...
            if self.n_task>1:
                ...
                self.sample_size_list_without_current_class=[quotient for _ in range(self.n_old_task)]
                for i in range(remainder):
                    self.sample_size_list_without_current_class[i]=self.sample_size_list[i]+1
        ...
        for iter_i in range(self.n_iter):
            ...
            #----update model on the old data----#
                ...
                # sample from each task including current task
                if self.sampled_memory_data is not None:
                    ...
                    for index in range(self.n_old_task):
                        task_id=self.observed_tasks[index]
                        sample_size=self.sample_size_list_without_current_class[index]
                        buffer_size=self.sampled_task_labs[task_id].size(0)
                        random_inds=random.sample(range(0,buffer_size),sample_size)
