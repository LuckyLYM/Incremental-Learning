...
class Net(nn.Module):
    ...
    def update_reservoir(self):
        ...
        if self.sampled_memory_data is None:
            ...
            # the reservoir pool is full
            if sampled_memory_size==self.n_sampled_memories:
                for i in range(self.n_memories):
                    current_data=self.memory_data[i]
                    current_label=self.memory_labs[i]
            ...
            elif total_size<=self.n_sampled_memories:
                ...
                effective_size=total_size-self.n_sampled_memories

                self.sampled_memory_data = torch.cat(
                    (self.sampled_memory_data, self.memory_data[:effective_size]), dim=0)
                self.sampled_memory_labs = torch.cat(
                    (self.sampled_memory_labs, self.memory_labs[:effective_size]), dim=0)

                for i in range(effective_size,self.n_memories):
                    current_data=self.memory_data[i]
                    current_label=self.memory_labs[i]
                    j = np.random.randint(0, self.examples_seen_so_far)
                    if j<self.n_sampled_memories:
