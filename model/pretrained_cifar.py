...
def cifar_resnet20(pretrained=None,num_classes=0, **kwargs):
    if pretrained is None:
        ...
        model.load_state_dict(model_zoo.load_url(pretrained_settings[pretrained]['resnet20'],model_dir='./'))
        if num_classes!=0:
...
def cifar_resnet32(pretrained=None,num_classes=0, **kwargs):
    if pretrained is None:
        ...
        model.load_state_dict(model_zoo.load_url(pretrained_settings[pretrained]['resnet32'],model_dir='./'))
        if num_classes!=0:
            num_ftrs = model.fc.in_features
            model.fc=nn.Linear(num_ftrs,num_classes)
    return model
...
def cifar_resnet44(pretrained=None,num_classes=0, **kwargs):
    if pretrained is None:
...
def cifar_resnet56(pretrained=None,num_classes=0, **kwargs):
    if pretrained is None:
